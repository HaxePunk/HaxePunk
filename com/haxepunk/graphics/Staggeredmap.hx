 ...
 */
class Staggeredmap extends Canvas
{
	/**
	...
	*/
	...
	public override function render(target:BitmapData, point:Point, camera:Point)
	{
		...
		var wx = (_point.x + startx * tw) * scalex - Std.int(tw * scalex / 2),
		...
		for (y in starty...desty)
		{
			...
			{
				tile = _map[y % _map.length][x % _map[0].length];
				if (tile >= 0)
				{
					if (_blit)
					{
			...
			wx = (_point.x + startx * tw) * scalex - (( y%2 == 0 ) ? 0 : Std.int(tw * scalex / 2) );
			wy += Std.int(th * scaley / 2);
		}
		if (_blit)
			target.unlock();
	}
