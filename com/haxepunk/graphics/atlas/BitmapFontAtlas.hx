...
class BitmapFontAtlas extends TextureAtlas
{
	...
	public static function getFont(fontName:String, ?format:BitmapFontFormat, ?extraParams:Dynamic):BitmapFontAtlas
	{
		...
		if (fontName == HXP.defaultFont + ".png") format = PIXELIZER;

		if (!_fonts.exists(fontName))
		{
			_fonts[fontName] = switch(format) {
				case XML: BitmapFontAtlas.loadXMLFont(fontName);
				case PIXELIZER: BitmapFontAtlas.loadPixelizerFont(fontName, extraParams);
			}
		}
	...
	/*
	 * Load a font from Pixelizer format.
	...
	public static function loadPixelizerFont(asset:String, ?options:Dynamic):BitmapFontAtlas
	{
		var atlas = new BitmapFontAtlas(asset);
	
	#if flash
		var bmd:BitmapData = atlas._data._tilesheet.nmeBitmap;
	#else
		var bmd:BitmapData = atlas._data._tilesheet.__bitmap;
	#end
	
		if (bmd == null) throw 'Invalid font asset "$asset": no BitmapData found.';
